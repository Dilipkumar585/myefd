// =============================================
// EFD Mechanic Booking - Premium v2.0
...
const bookBtn = document.getElementById("bookMechanic");

// ===== AUTO-DETECT LOCATION ON PAGE LOAD =====
(function autoDetectLocation() {
    const locationInput = document.getElementById("location2");
    const locationStatus = document.getElementById("locationStatus2");
    const locationIcon = document.getElementById("locationIcon2");
...
bookBtn.addEventListener("click", function() {
    ...
    if (vehicleType === "" || problem === "" || location === "" || phone === "") {
    ...
    navigator.geolocation.getCurrentPosition(
        async function(position) {
            ...
            try {
                ...
                showToast("Mechanic request sent! 🎉", "success");
                ...
                bookBtn.innerHTML = '<i class="fa-solid fa-tools"></i> Request Mechanic';
            }
        },
        function(error) {
...
// ===== TIMELINE TRACKING (same as fuel-booking) =====
    ...
    if (order.providerId) {
        ...
        getDoc(doc(db, "users", order.providerId)).then((agentDoc) => {
            if (agentDoc.exists()) {
                ...
                if (serviceTypeEl) serviceTypeEl.textContent = "Mechanic Specialist";
...
function updateETA(order) {
    ...
    if (order.status === "Accepted") {
        ...
        etaText.textContent = order.eta ? `Arriving in ~${order.eta} mins` : "Agent is on the way";
